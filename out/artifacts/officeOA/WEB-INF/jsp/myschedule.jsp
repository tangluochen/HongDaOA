	...
	<div class="page-container">
		...
		<div class="text-c">
			...
			搜索条件： <input type="text" class="input-text" style="width: 250px"
				placeholder="输入日程名字" id="schedulename1" name="schedulename1">
...
<%--						</td>--%>
						<td class="td-manage"><a title="编辑"
							href="selectupdate.do?tScheduleId=${myschedules.tScheduleId }"
							class="ml-5" style="text-decoration: none"
							onclick="bianji(${myschedules.tScheduleCreateuser },${sessionScope.TUserinfo.tUserinfoName})"><i
								class="Hui-iconfont">&#xe6df;</i></a>&nbsp; <a title="删除" id="${myschedules.tScheduleId}"
		...
		<div align="right">
			<font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第
				${page.pageNow} 页</font> <a href="my_schedule.do?pageNow=1">首页</a>
			<c:choose>
				<c:when test="${page.pageNow - 1 > 0}">
					<a href="my_schedule.do?pageNow=${page.pageNow - 1}">上一页</a>
				</c:when>
				<c:when test="${page.pageNow - 1 <= 0}">
					<a href="my_schedule.do?pageNow=1">上一页</a>
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${page.totalPageCount==0}">
					<a href="my_schedule.do?pageNow=${page.pageNow}">下一页</a>
				</c:when>
				<c:when test="${page.pageNow + 1 < page.totalPageCount}">
					<a href="my_schedule.do?pageNow=${page.pageNow + 1}">下一页</a>
				</c:when>
				<c:when test="${page.pageNow + 1 >= page.totalPageCount}">
					<a href="my_schedule.do?pageNow=${page.totalPageCount}">下一页</a>
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${page.totalPageCount==0}">
					<a href="my_schedule.do?pageNow=${page.pageNow}">尾页</a>
				</c:when>
				<c:otherwise>
					<a href="my_schedule.do?pageNow=${page.totalPageCount}">尾页</a>
				</c:otherwise>
			</c:choose>
		</div>
		<!-- 分页功能 End -->
